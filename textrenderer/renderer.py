...
class Renderer(object):
    ...
    def gen_img(self):
        ...
        bg = self.gen_bg(width=word_size[0] * self.cfg.scale.width, height=word_size[1] * self.cfg.scale.height)

        reword = self.replace_word(word)
        word_img, text_box_pnts, word_color = self.draw_text_on_bg(reword, font, bg)
    ...
    def get_word_color(self, bg, text_x, text_y, word_height, word_width):
        ...
        if len(word_roi_bg.shape)==3:
            bg_mean = word_roi_bg.reshape((-1, 3)).mean(axis=0)
            word_color = []
            for i in range(0, bg_mean.size):
                if bg_mean[i] > 128:
                    word_color.append(random.randint(0, int(bg_mean[i]-64)))
                else:
                    word_color.append(random.randint(int(bg_mean[i]+64), 255))
            return tuple(word_color)
        else:
    ...
    def draw_text_seamless(self, font, bg, word, word_color, word_height, word_width, offset):
        ...
        if self.cfg.color.enable:
            ...
            text_img_bgr = cv2.cvtColor(text_img, cv2.COLOR_GRAY2BGR)
    ...
    def draw_border_text(self, draw, text, x, y, font, text_color):
        ...
        text_color_np = np.asarray(text_color)
        mean_color = text_color_np.mean()

        if light_or_dark == 0:
            border_color = text_color_np + np.random.uniform(0, 255 - mean_color - 1, text_color_np.shape)
        elif light_or_dark == 1:
            border_color = text_color_np - np.random.uniform(0, mean_color + 1, text_color_np.shape)
        border_color = np.uint8(border_color)
        border_color = tuple(border_color.reshape(1, -1)[0])
        # thin border
    ...
    def gen_rand_bg(self, width, height):
        ...
        """
        alpha = np.random.randint(64, 255)
        if self.cfg.color.enable:
            bg = alpha*np.ones((height, width, 3), np.uint8)
        else:
            bg = alpha*np.ones((height, width), np.uint8)
        bg += np.random.randint(-32, 32, bg.shape).astype(np.uint8)
        bg = np.uint8(self.apply_gauss_blur(bg))
        return bg
    ...
    def gen_bg_from_image(self, width, height):
        ...
        if self.cfg.color.enable == False:
        ...
        beta = 10
        alpha = 0.8
        alpha = np.random.uniform(alpha, 1.0/alpha)      # 对比度
        beta = np.random.randint(-beta, beta)  # 亮度
        out = np.uint8(np.clip((alpha * out + beta), 0, 255))

        return out
    chi_eng_default =  {"０": "0", "１": "1", "２": "2", "３": "3", "４": "4", "５": "5", "６": "6", "７": "7", "８": "8", "９": "9", 
                ...
                "ｙ": "y", "ｚ": "z", "．": "."}
    def replace_word(self, word, chi_eng = chi_eng_default):
        reword = ""
        for char in word:
            if char in chi_eng.keys():
                reword += chi_eng[char]
            else:
                reword += char
        return reword
    ...
    def apply_blur_on_output(self, img):
        if prob(0.5):
            return self.apply_gauss_blur(img, [5, 7, 9])
        else:
