...
if flags.corpus_mode == 'sohu':
    chars = utils.load_dicts(flags.chars_file+".dict")
else:
    chars = utils.load_dicts(flags.chars_file)
...
def sort_labels(tmp_label_fname, label_fname, char_dict):
    lines = []
    ...
    with open(label_fname, mode='w', encoding='utf-8') as f:
        for line in lines:
            base_name, word = line.split()
            labels=""
            for i, char in enumerate(word):
                classid = char_dict[char]
                labels += " %s"%str(classid)
            f.write("%s.jpg%s\n"%(base_name, labels))
...
if __name__ == "__main__":
    ...
    if not flags.viz:
        sort_labels(tmp_label_path, label_path, chars)
